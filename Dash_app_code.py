...
def get_scatter_chart(entered_site, slider_values):
    min_slider, max_slider = slider_values 
    mask = (spacex_df['Payload Mass (kg)'] >= min_slider) & (spacex_df['Payload Mass (kg)'] <= max_slider)
    masked_df = spacex_df[mask]

    if entered_site == 'ALL': 
        fig = px.scatter(masked_df, x = 'Payload Mass (kg)', y = 'class', 
            color = 'Booster Version Category', 
            title = 'Correlation between Payload and Success for all Sites')
        return fig
    
    else: 
        site_df = masked_df[masked_df['Launch Site'] == entered_site]
        fig = px.scatter(site_df, x = 'Payload Mass (kg)', y = 'class', 
            color = 'Booster Version Category', 
            title = 'Correlation between Payload and Success for site {site}'.format(site = entered_site))
